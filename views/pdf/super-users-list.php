...
$selectSQL2 = "SELECT * FROM super_user_tbl";
    ...
    <tbody>
        <tr class="headerColorMain" style="background-color: #fff !important;">
          <td style="width:30%;">
			  ...
			  <h4 style="text-align:left !important;font-weight:">Super User List</h4>
		  </td>
          <td style="width:30%;">
...
while($row2 = mysqli_fetch_assoc($result2)){
	$super_user_name = ''.$row2['first_name'].' '.$row2['last_name'].'';
	$nic = $row2['nic'];
	$mobile = $row2['mobile'];
	$gender = $row2['gender'];
	$address = $row2['address'];
	$output .= '<tr>
					<th scope="row">'.$super_user_name.'</th>
					  <td>'.$nic.'</td>
					  <td>'.$mobile.'</td>
					  <td>'.$gender.'</td>
					  <td>'.$address.'</td>
...
//$output = $websiteurl;
	...
	$pdf->stream("bestproperty.lk_super_user_list.pdf", array("Attachment" => 0));
