...
$selectSQL2 = "SELECT * FROM companies_tbl";
...
//$sliderImg1 = $row2['slider_img_name'];	
	...
	$output .= '  <thead class="text-white" style="background-color: indigo !important;">
					<tr>
					  ...
					  <th scope="col">Com reg No</th>
					  <th scope="col">Com Phone</th>
					  <th scope="col">Com Email</th>
					  <th scope="col">Com Entered Date</th>
					</tr>
				  </thead>';
	$output .= '<tbody>';	
...
while($row2 = mysqli_fetch_assoc($result2)){
	...
	$company_reg_no = $row2['company_reg_no'];
	$company_phone = $row2['company_phone'];
	$company_email = $row2['company_email'];
	$company_entered_date = $row2['company_entered_date'];
	$output .= '<tr>
					<th scope="row">'.$company_id.'</th>
					  <td>'.$company_name.'</td>
					  <td>'.$company_reg_no.'</td>
					  <td>'.$company_phone.'</td>
					  <td>'.$company_email.'</td>
					  <td>'.substr($company_entered_date,0,10).'</td>
					</tr>';	
}
