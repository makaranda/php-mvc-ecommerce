...
if($signature_status == true)
{
	...
	//var_dump($responseVariables);
		echo '<br/>';
	//ec//ho 'Returned session-' ;
	//print_r($_SESSION['sample_test']);
	//unset($_SESSION['user_details']);
	header('Location: assets/action/process_order_online.php?ordmsg=success');
}else
{
	//echo 'Error Validation'; 
	//unset($_SESSION['user_details']);
	//echo '<script>window.open("'.URL.'invoice/", "_blank");</script>';
	header('Location: assets/action/process_order_online.php?ordmsg=notsuccess');
}
